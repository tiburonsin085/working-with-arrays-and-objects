...
//Code Here
let evenNumbers = mixedNumbers.filter(n => n%2 ===0) // = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map(n => n*1.07) // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((acc,x)=>acc+x)//  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter( n => n['CP'] > 200)// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map( n => (n['price']*(1+n['tax'])))// Code here
...
let bobsTotal = (purchases.filter( element => (element['owner'] === 'Bob')))//.reduce( (acc,x) => acc + x['price'] ))
//Code Here

bobsTotal  = (bobsTotal.map((x)=> x['price'])).reduce((acc,n)=> acc + n )
